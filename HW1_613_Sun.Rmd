...
Full_att = left_join(Full_Data,entry_exit, by = "idmen") %>% group_by(year) %>% 
  summarise(full_count = n(), attrition_count = sum(attrition, na.rm=TRUE)) %>% 
  filter(year >2004 & year <2019) %>% 
  mutate(attrition_ratio = attrition_count/full_count) %>% select(year,attrition_ratio)
