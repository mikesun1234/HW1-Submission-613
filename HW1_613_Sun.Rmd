...
ggplot(datind2010_ageclean) + aes(age) + geom_histogram(binwidth=5) + ggtitle("2010 Population Count by Age with Bin size = 4")

male_age_2010 <- datind2010_ageclean %>% filter( gender == "Male" ) %>% ggplot( aes(x=age)) +
    geom_histogram( binwidth=4, fill="#69b3a2", color="#69b3a2") + ggtitle("2010 Male Count by Age with Bin size = 4")

female_age_2010 <- datind2010_ageclean %>% filter( gender == "Female" ) %>% ggplot( aes(x=age)) +
    geom_histogram( binwidth=4, fill = "blue", color="blue") + ggtitle("2010 Female Count by Age with Bin size = 4")
...
```{r}
Same_Profession = Full_Data %>% group_by(idmen, year, profession) %>% mutate(n_member = n()) %>% filter(n_member > 2)
...
```{r}
entry_exit <- Full_Data %>% group_by(idmen) %>% summarize(entry = min(year), exit = max(year)) %>% mutate(time_spent = (exit - entry +1))
```
...
```{r}
hh_year_ratio = hh_year %>% group_by(year) %>% summarise(total_c = n(), move_c = sum(move_in_check,na.rm = T),m_ratio = move_c/total_c)
```

```{r}
ggplot(hh_year_ratio, aes(y=m_ratio,x=year)) + geom_line() + ggtitle("Move in by Year") + xlab("Year") + ylab("Share of Moved in Popualtion")
```
...
```{r}
migrate_year = Full_Data %>% mutate(mi_check = ifelse(is.na(move) == F & move ==2, 1, ifelse(year == myear, 1 , 0)))

head(migrate_year, 10)
```

```{r}
migrate_year_share = migrate_year %>% group_by(year) %>% summarise(year_count = n(), mi_count = sum(mi_check,na.rm = TRUE)) %>% mutate(mi_share = mi_count/year_count)
```

```{r}
ggplot(migrate_year_share, aes(y=mi_share,x=year)) + geom_line() + ggtitle("Migrate by Year") + xlab("Year") + ylab("Share of Migrated Population")
```
...
```{r, include=FALSE}
by_plot = left_join(migrate_year_share, hh_year_ratio, by = "year") %>% select(year, mi_share, m_ratio)
```
...
```{r}
prof_emp_c = migrate_year %>% filter(mi_check == 1) %>% group_by(idmen,profession,empstat) %>% mutate(check = n()) %>% mutate(ifelse(check != 1, 1,0)) %>% filter(check == 1)
```
...
Full_att = left_join(Full_Data,entry_exit, by = "idmen") %>% group_by(year) %>% summarise(full_count = n(), attrition_count = sum(attrition, na.rm=TRUE)) %>% filter(year >2004 & year <2019) %>% mutate(attrition_ratio = attrition_count/full_count) %>% select(year,attrition_ratio)
Full_att
```
